...
export async function GET(_: Request, context: RouteContext) {
  ...
  return NextResponse.json({
    items: result.snapshot.evidencePacks,
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const result = await createEvidencePackForGrant(slug, {
    windowLabel: String(payload.windowLabel ?? ""),
    githubRepoUrl: String(payload.githubRepoUrl ?? ""),
    githubBranch: String(payload.githubBranch ?? ""),
    githubCommitHash: String(payload.githubCommitHash ?? ""),
    docsUrl: String(payload.docsUrl ?? ""),
    demoUrl: String(payload.demoUrl ?? ""),
    testRunUrl: String(payload.testRunUrl ?? ""),
  ...
  const redirectUrl = new URL(`/dashboard/grants/${slug}`, request.url);
  redirectUrl.searchParams.set("actionSuccess", "evidence");
  return NextResponse.redirect(redirectUrl, { status: 303 });
}
